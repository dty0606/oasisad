...
#' @description This function will call fslbet_robust from extrantsr package
#' Skull Stripping (using FSL's BET) a file using \code{fslr}
#' functions and robustified by registration and neck removal
#' @param img (character) File to be skull stripped or object of class nifti
...
#' @param bet.opts Options for \code{{fslbet}}
#' @param nvoxels Number of voxels to dilate/erode.  See \code{{fslfill2}}.
#' If \code{nvoxels = 0}, then no smoothing is done.
#' @param swapdim Use \code{{fslswapdim}} to reorient image
#' @param remove.neck Run \code{{remove_neck}} to register the template to a
...
#' @param rbm.voxels Number of voxels to inflate mask for \code{{robust_brain_mask}}
...
#' @param ... additional arguments passed to \code{{robust_brain_mask}} or
#' \code{{remove_neck}}.
#' @return Skull-stripped \code{nifti} object
#' @note This function first thresholds an image, runs a rigid registration
#' (default in \code{{remove_neck}}) to drop any slices below the transformed
#' skull stripped template to remove neck slices.  The neck-removed image is
#' then skull stripped using defaults in \code{{fslbet}}.  A new
#' center of gravity is estiamted using \code{{cog}}, then the image is
#' skull stripped again using the new cog. After the skull stripped mask is
#' created, the image is dilated and eroded using \code{{fslfill2}} to
...
#' @importFrom neurobase check_outfile nii.stub
#' @importFrom oro.nifti cal_img drop_img_dim writeNIfTI
fslbet_rob <- function(
  img,
  outfile = NULL,
  retimg = TRUE,
